...
def dealer_turn(deck, dealer_cards)
  ...
  loop do
    ...
    break if bust?(dealer_total) || dealer_total >= DEALER_STAY
...
loop do
  ...
  loop do
    ...
    if calculate_winner(total(player_cards), total(dealer_cards)) == 'player'
      player_wins += 1
      break
    elsif calculate_winner(total(player_cards), total(dealer_cards)) == 'dealer'
      dealer_wins += 1
      break
    end
  end
  prompt("Score -- You #{player_wins} || Dealer #{dealer_wins}")
